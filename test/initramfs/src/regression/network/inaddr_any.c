...
#define GUEST_SECOND_VIRTIO_ADDR "10.0.3.15"
...
static bool local_addr_is_available(const char *ip)
{
	...
	CHECK(close(fd));
	CHECK_WITH(bind_result,
		   _ret == 0 || bind_errno == EADDRNOTAVAIL);
...
FN_TEST(tcp_inaddr_any_accepts_second_virtio_addr)
{
	...
	int connect_fd = TEST_SUCC(socket(AF_INET, SOCK_STREAM | SOCK_NONBLOCK, 0));
	init_addr(&connect_addr, GUEST_SECOND_VIRTIO_ADDR, bind_addr.sin_port);
	TEST_ERRNO(connect(connect_fd, (struct sockaddr *)&connect_addr,
			   sizeof(connect_addr)),
		   EINPROGRESS);
	...
	TEST_SUCC(getsockname(accepted_fd, (struct sockaddr *)&accepted_local_addr,
			      &accepted_local_addrlen));
	TEST_RES(accepted_local_addr.sin_addr.s_addr,
		 _ret == inet_addr(GUEST_SECOND_VIRTIO_ADDR));
	TEST_RES(write(connect_fd, &buf, sizeof(buf)), _ret == sizeof(buf));
	TEST_RES(read(accepted_fd, &buf, sizeof(buf)),
		 _ret == sizeof(buf) && buf == 'm');
...
FN_TEST(tcp_inaddr_any_connects_via_second_virtio)
{
	SKIP_TEST_IF(!local_addr_is_available(GUEST_SECOND_VIRTIO_ADDR));
	...
	int listen_fd = TEST_SUCC(socket(AF_INET, SOCK_STREAM | SOCK_NONBLOCK, 0));
	init_addr(&listen_addr, GUEST_SECOND_VIRTIO_ADDR, 0);
	...
	TEST_SUCC(getsockname(client_fd, (struct sockaddr *)&client_bind_addr,
			      &client_bind_addrlen));
	TEST_RES(client_bind_addr.sin_addr.s_addr, _ret == htonl(INADDR_ANY));
	...
	TEST_RES(client_local_addr.sin_addr.s_addr,
		 _ret == inet_addr(GUEST_SECOND_VIRTIO_ADDR));
	TEST_RES(client_local_addr.sin_port,
		 _ret == client_bind_addr.sin_port);
	TEST_RES(write(client_fd, &buf, sizeof(buf)), _ret == sizeof(buf));
	TEST_RES(read(accepted_fd, &buf, sizeof(buf)),
		 _ret == sizeof(buf) && buf == 'p');
...
FN_TEST(tcp_inaddr_any_ephemeral_skips_second_iface_conflict)
{
	...
	int blocker_fd = TEST_SUCC(socket(AF_INET, SOCK_STREAM, 0));
	init_addr(&blocker_addr, GUEST_SECOND_VIRTIO_ADDR, 0);
	TEST_SUCC(bind(blocker_fd, (struct sockaddr *)&blocker_addr,
		       sizeof(blocker_addr)));
	TEST_SUCC(getsockname(blocker_fd, (struct sockaddr *)&blocker_addr,
			      &blocker_addrlen));

	int wildcard_fd = TEST_SUCC(socket(AF_INET, SOCK_STREAM, 0));
...
FN_TEST(tcp_inaddr_any_conflict_rolls_back)
{
	struct sockaddr_in loopback_addr;
	struct sockaddr_in wildcard_addr;
	struct sockaddr_in virtio_addr;
	const in_port_t port = htons(31002);
	...
	int virtio_fd = TEST_SUCC(socket(AF_INET, SOCK_STREAM, 0));
...
FN_TEST(udp_inaddr_any_receives_second_virtio_addr)
{
	SKIP_TEST_IF(!local_addr_is_available(GUEST_SECOND_VIRTIO_ADDR));
	...
	int send_fd = TEST_SUCC(socket(AF_INET, SOCK_DGRAM, 0));
	init_addr(&send_addr, GUEST_SECOND_VIRTIO_ADDR, bind_addr.sin_port);
	TEST_RES(sendto(send_fd, &send_buf, sizeof(send_buf), 0,
			(struct sockaddr *)&send_addr, sizeof(send_addr)),
		 _ret == sizeof(send_buf));
	...
	TEST_RES(recvfrom(recv_fd, &recv_buf, sizeof(recv_buf), 0,
			  (struct sockaddr *)&source_addr, &source_addrlen),
		 _ret == sizeof(recv_buf) && recv_buf == 'n');
	TEST_RES(source_addr.sin_addr.s_addr,
		 _ret == inet_addr(GUEST_SECOND_VIRTIO_ADDR));
...
FN_TEST(udp_inaddr_any_sends_via_second_virtio)
{
	SKIP_TEST_IF(!local_addr_is_available(GUEST_SECOND_VIRTIO_ADDR));
	...
	int recv_fd = TEST_SUCC(socket(AF_INET, SOCK_DGRAM | SOCK_NONBLOCK, 0));
	init_addr(&recv_addr, GUEST_SECOND_VIRTIO_ADDR, 0);
	TEST_SUCC(bind(recv_fd, (struct sockaddr *)&recv_addr, sizeof(recv_addr)));
	TEST_SUCC(getsockname(recv_fd, (struct sockaddr *)&recv_addr, &recv_addrlen));
	...
	TEST_RES(recvfrom(recv_fd, &recv_buf, sizeof(recv_buf), 0,
			  (struct sockaddr *)&source_addr, &source_addrlen),
		 _ret == sizeof(recv_buf) && recv_buf == 'o');
	TEST_RES(source_addr.sin_addr.s_addr,
		 _ret == inet_addr(GUEST_SECOND_VIRTIO_ADDR));
	TEST_RES(source_addr.sin_port, _ret == send_bind_addr.sin_port);
