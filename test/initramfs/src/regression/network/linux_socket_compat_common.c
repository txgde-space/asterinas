...
#define ASTERINAS_PRIMARY_ADDR "10.0.2.15"
#define COMMON_RESPONSE_SIZE (64 * 1024)
...
static const char *primary_ipv4_addr(void)
{
	const char *env_addr = getenv("SOCKET_COMPAT_PRIMARY_ADDR");

	if (env_addr != NULL && env_addr[0] != '\0')
		return env_addr;

#ifdef __asterinas__
	return ASTERINAS_PRIMARY_ADDR;
#else
	static char detected_addr[INET_ADDRSTRLEN];
	struct sockaddr_in remote_addr;
	struct sockaddr_in local_addr;
	socklen_t local_addrlen = sizeof(local_addr);
	int probe_fd;

	init_addr(&remote_addr, "8.8.8.8", htons(53));
	probe_fd = CHECK(socket(AF_INET, SOCK_DGRAM, 0));
	CHECK(connect(probe_fd, (struct sockaddr *)&remote_addr,
		      sizeof(remote_addr)));
	CHECK(getsockname(probe_fd, (struct sockaddr *)&local_addr,
			  &local_addrlen));
	CHECK(close(probe_fd));
	CHECK_WITH(inet_ntop(AF_INET, &local_addr.sin_addr, detected_addr,
			     sizeof(detected_addr)),
		   _ret != NULL);
...
static int read_exact(int fd, char *buf, size_t len)
{
	...
	while (received < len) {
		ssize_t ret = read(fd, buf + received, len - received);
...
FN_TEST(tcp_inaddr_any_getsockname_and_connect)
{
	...
	init_addr(&connect_addr, primary_ipv4_addr(), bind_addr.sin_port);
...
FN_TEST(udp_inaddr_any_receives_datagram)
{
	...
	init_addr(&send_addr, primary_ipv4_addr(), bind_addr.sin_port);
...
FN_TEST(tcp_listen_autobinds_inaddr_any)
{
	...
	int connect_fd = TEST_SUCC(socket(AF_INET, SOCK_STREAM | SOCK_NONBLOCK, 0));
	init_addr(&connect_addr, primary_ipv4_addr(), listen_addr.sin_port);
	TEST_ERRNO(connect(connect_fd, (struct sockaddr *)&connect_addr,
			   sizeof(connect_addr)),
		   EINPROGRESS);
...
FN_TEST(common_service_response_transfer)
{
	struct sockaddr_in listen_addr;
	char send_buf[COMMON_RESPONSE_SIZE];
	char recv_buf[COMMON_RESPONSE_SIZE];

	memset(send_buf, 'h', sizeof(send_buf));
	int listen_fd = new_loopback_listener(&listen_addr, 1);
	int client_fd = connect_loopback(&listen_addr);
	int accepted_fd = TEST_SUCC(accept(listen_fd, NULL, NULL));

	TEST_RES(write(accepted_fd, send_buf, sizeof(send_buf)),
		 _ret == sizeof(send_buf));
	TEST_SUCC(read_exact(client_fd, recv_buf, sizeof(recv_buf)));
	TEST_RES(memcmp(send_buf, recv_buf, sizeof(send_buf)), _ret == 0);
