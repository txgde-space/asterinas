...
static ssize_t write_all(int fd, const void *buf, size_t len)
{
	...
	while (done < len) {
		ssize_t ret = send(fd, (const char *)buf + done, len - done, 0);
...
FN_TEST(tcp_read_wrap_receive_buffer_tail)
{
	...
	TEST_RES(write_all(server_fd, buf, two_thirds_buf_len),
		 _ret == (ssize_t)two_thirds_buf_len);
	usleep(TCP_SETTLE_USEC);
	TEST_RES(read_exact(client_fd, buf, one_third_buf_len),
	...
	for (int i = 0; i < RECEIVE_BUFFER_FILL_DRAIN_ROUNDS; i++) {
		TEST_RES(write_all(server_fd, buf, two_thirds_buf_len),
			 _ret == (ssize_t)two_thirds_buf_len);
		usleep(TCP_SETTLE_USEC);
		TEST_RES(read_exact(client_fd, buf, two_thirds_buf_len),
			 _ret == (ssize_t)two_thirds_buf_len);
		usleep(TCP_SETTLE_USEC);
	}
